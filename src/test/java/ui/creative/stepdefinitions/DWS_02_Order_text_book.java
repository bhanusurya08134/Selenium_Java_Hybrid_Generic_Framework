...
public class DWS_02_Order_text_book extends BaseTest {

	TestSetup testSetup;

	public DWS_02_Order_text_book(TestSetup testSetup) throws IOException {
		this.testSetup = testSetup;
	}

	@Then("I select book and add to cart {string}")
	public void i_select_book_and_add_to_cart(String tcid) {
		try {
...
//			String fieldId = tcid.split("_")[0]+"_"+tcid.split("_")[1] ;
			...
			String confirmationMessage = testSetup.pageObjectManager.getBooksPage().selectBookAndAddToCart(testData);
	...
	@Then("I order the book {string}")
	public void i_order_the_book(String tcid) {
		try {
...
//			String fieldId = tcid.split("_")[0]+"_"+tcid.split("_")[1] ;
			...
			if(orderConfirm[0].equalsIgnoreCase("Your order has been successfully processed!")) {
				...
				reportInfo("'"+testData.get("BookName")+"'  Book ordered successfully with "+orderConfirm[1]);
			}
			else {
